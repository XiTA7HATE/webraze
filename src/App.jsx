...
export default function App(){
  ...
  useEffect(()=>{
    ...
    if(tg){
      ...
      if(tg.initDataUnsafe?.user?.photo_url) setAvatar(tg.initDataUnsafe.user.photo_url);
    }
    loadData();
  },[]);

  async function apiFetch(path,opts={}){
    const res=await fetch(API+path,{...opts,headers:{"x-init-data":initData,"Content-Type":"application/json",...(opts.headers||{})}});
  ...
  async function loadData(){
    try{
      ...
      setUser({calories_goal:2768,protein_goal:191,fat_goal:86,carb_goal:293,weight:75,height:180,age:22});
    }
  }

  async function postMeal(meal){
    try{ await apiFetch("/api/meals",{method:"POST",body:JSON.stringify(meal)}); }
    catch(e){ console.log("offline, local only"); }
    const now=new Date();
    const time=`${now.getHours().toString().padStart(2,"0")}:${now.getMinutes().toString().padStart(2,"0")}`;
    setMeals(p=>[...p,{...meal,id:Date.now(),time}]);
  }

  async function postWater(amount){
    try{ await apiFetch("/api/water",{method:"POST",body:JSON.stringify({amount})}); }
    catch(e){}
    setWater(w=>w+amount);
  }

  const totals=meals.reduce((a,m)=>({cal:a.cal+m.calories,prot:a.prot+(m.protein||0),fat:a.fat+(m.fat||0),carbs:a.carbs+(m.carbs||0)}),{cal:0,prot:0,fat:0,carbs:0});
  ...
  async function analyzeFood(){
    ...
    try{
      const res=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-6",max_tokens:1000,messages:[{role:"user",content:[{type:"image",source:{type:"base64",media_type:"image/jpeg",data:imageBase64}},{type:"text",text:`Ты эксперт по питанию. Посмотри на фото еды и определи КБЖУ. Ответь СТРОГО в JSON (только JSON без markdown):
...
Если не еда: {"error":"not_food"}`}]}]})});
  ...
  async function analyzeBarcode(){
    ...
    setAnalyzing(true);setResult(null);
    try{
      const res=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-6",max_tokens:200,messages:[{role:"user",content:[{type:"image",source:{type:"base64",media_type:"image/jpeg",data:imageBase64}},{type:"text",text:"На фото штрихкод. Прочитай цифры и ответь ТОЛЬКО цифрами без пробелов. Например: 4607026570124"}]}]})});
      const data=await res.json();
      ...
      if(offData.status!==1){setResult({error:`Продукт не найден 😔`});setAnalyzing(false);return;}
  ...
  async function analyzeText(){
    ...
    setAnalyzing(true);setResult(null);
    try{
      const res=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-6",max_tokens:1000,messages:[{role:"user",content:`Ты эксперт по питанию. Пользователь написал: "${textInput}". Ответь СТРОГО в JSON (только JSON без markdown):
{"name":"название","portion":граммы,"calories":число,"protein":число,"fat":число,"carbs":число,"tip":"совет"}
Если не еда: {"error":"not_food"}`}]})});
      const data=await res.json();
      const parsed=JSON.parse(data.content[0].text.replace(/```json|```/g,"").trim());
  ...
  async function addToLog(){
    ...
    await postMeal({meal_name:result.name,portion:result.portion,calories:result.calories,protein:result.protein,fat:result.fat,carbs:result.carbs});
  ...
  const goal=user||{calories_goal:2768,protein_goal:191,fat_goal:86,carb_goal:293};
    ...
    <div style={{minHeight:"100vh",background:"#060606",color:"#fff",fontFamily:"'Inter',-apple-system,sans-serif",maxWidth:480,margin:"0 auto",position:"relative"}}>
      ...
      {tab===0&&(
        <div style={S.page}>
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:20}}>
            ...
            <CalorieRing eaten={totals.cal} goal={goal.calories_goal}/>
            <div style={{textAlign:"center"}}>
              <div style={{fontSize:20,fontWeight:800,color:totals.cal>goal.calories_goal?"#f87171":"#fff"}}>{Math.max(0,goal.calories_goal-totals.cal)}</div>
              <div style={{fontSize:11,color:"#555"}}>ккал</div>
          ...
          <div style={S.card}>
            ...
            <div style={{display:"flex",justifyContent:"space-around"}}>
              <MiniRing value={totals.prot} max={goal.protein_goal} color="#60a5fa" label="Белки"/>
              <MiniRing value={totals.carbs} max={goal.carb_goal} color="#a78bfa" label="Углеводы"/>
              <MiniRing value={totals.fat} max={goal.fat_goal} color="#fbbf24" label="Жиры"/>
            </div>
          </div>
          ...
          <div style={S.card}>
            ...
            <div style={{display:"flex",gap:6}}>
              {[150,200,250,500].map(ml=>(
                <button key={ml} onClick={()=>postWater(ml)} style={{flex:1,padding:"8px 0",background:"#0d0d0d",border:"1px solid #1c1c1c",borderRadius:8,color:"#60a5fa",fontSize:12,cursor:"pointer",fontWeight:600}}>+{ml}</button>
              ))}
            </div>
          </div>
      ...
      {tab===1&&(
        <div style={S.page}>
          ...
          {(addMode==="food"||addMode==="barcode")&&(
            <>
              <input ref={addMode==="food"?foodRef:barcodeRef} type="file" accept="image/*" onChange={handleFile} style={{display:"none"}}/>
              {!imagePreview?(
                <div onClick={()=>(addMode==="food"?foodRef:barcodeRef).current.click()} style={{...S.card,textAlign:"center",padding:"48px 20px",cursor:"pointer",border:"2px dashed #1c1c1c"}}>
                  <div style={{fontSize:52,marginBottom:12}}>{addMode==="food"?"📷":"〓"}</div>
                  <div style={{color:"#555",fontSize:14}}>{addMode==="food"?"Сфотографируй еду":"Сфотографируй штрихкод"}</div>
                  {addMode==="barcode"&&<div style={{color:"#333",fontSize:12,marginTop:8}}>AI прочитает цифры → найдёт продукт</div>}
                </div>
              ):(
              ...
              {imagePreview&&!result&&<button onClick={addMode==="food"?analyzeFood:analyzeBarcode} disabled={analyzing} style={{...S.btn(),marginTop:8,opacity:analyzing?0.6:1}}>{analyzing?(addMode==="food"?"⏳ Анализирую...":"⏳ Читаю штрихкод..."):(addMode==="food"?"🔍 Определить КБЖУ":"🔍 Найти продукт")}</button>}
      ...
      {tab===2&&(
        <div style={S.page}>
          ...
          {history.length===0?(
            <div style={{...S.card,textAlign:"center",padding:"40px",color:"#444"}}>История пуста</div>
          ):history.map(({day,cal})=>{
            const pct=Math.min(cal/goal.calories_goal*100,100),over=cal>goal.calories_goal;
            const dayStr=typeof day==="string"?day:new Date(day).toLocaleDateString("ru",{day:"2-digit",month:"2-digit"});
            return(
              <div key={dayStr} style={S.card}>
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
                  <span style={{fontWeight:600,fontSize:14}}>{dayStr}</span>
                ...
                <div style={{fontSize:11,color:"#333",marginTop:4}}>{over?`⚠️ +${cal-goal.calories_goal} ккал`:`✅ −${goal.calories_goal-cal} ккал`}</div>
              </div>
            );
          })}
        </div>
      ...
      {tab===3&&(
        <div style={S.page}>
          ...
          <div style={{...S.card,textAlign:"center",paddingTop:28,paddingBottom:24}}>
            ...
            <div style={{fontSize:22,fontWeight:900}}>{window.Telegram?.WebApp?.initDataUnsafe?.user?.first_name||"Пользователь"}</div>
          </div>
          <div style={S.card}>
            <div style={{fontSize:12,color:"#555",fontWeight:700,marginBottom:12,letterSpacing:1}}>ДНЕВНАЯ НОРМА</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
              {[["🔥","Калории",goal.calories_goal,"ккал","#22c55e"],["🥩","Белки",goal.protein_goal,"г","#60a5fa"],["🧈","Жиры",goal.fat_goal,"г","#fbbf24"],["🍞","Углеводы",goal.carb_goal,"г","#a78bfa"]].map(([ic,l,v,u,c])=>(
          ...
          <div style={{...S.card,textAlign:"center",color:"#444",fontSize:13}}>Для изменения нормы используй Telegram-бота</div>
        </div>
      )}
